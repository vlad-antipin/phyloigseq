...
# scratch. See unifrac-optimization.Rmd for the derivation/benchmarks against
# phyloseq::distance().
#
...
sparse_unifrac <- function(physeq, method = c("unifrac", "wunifrac")) {
  ...
  # edge_abundance[e, sample] = descendant-tip abundance total for the edge
  # leading into tree$edge[e, 2] (i.e. the "child end" of edge e).
  edge_abundance <- subtree_abundance[tree$edge[, 2], , drop = FALSE]
  ...
  # Unweighted UniFrac for every sample pair at once, via matrix
  # multiplication instead of a per-pair intersect/union loop. With
  # present_e,s the 0/1 indicator of whether edge e has nonzero descendant
  # abundance in sample s:
